...
template <typename T>
static __global__ void pad_kernel(const T * src, size_t s00, size_t s01, size_t s02, size_t s03, T * dst,
...
template <typename T>
static void pad_cuda(const T * src, size_t s00, size_t s01, size_t s02, size_t s03, T * dst,
    const int lp0, const int rp0, const int lp1, const int rp1,
    const int lp2, const int rp2, const int lp3, const int rp3,
    const int ne0, const int ne1, const int ne2, const int ne3,
    ...
    pad_kernel<T><<<gridDim, CUDA_PAD_BLOCK_SIZE, 0, stream>>>(src, s00, s01, s02, s03, dst,
...
void ggml_cuda_op_pad(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src0->type == GGML_TYPE_F32) {
        pad_cuda<float>((const float *) src0->data, s00, s01, s02, s03, (float *) dst->data,
                        lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3,
                        dst->ne[0], dst->ne[1], dst->ne[2], dst->ne[3],
                        (bool) circular, stream);
    } else {
        pad_cuda<half>((const half *) src0->data, s00, s01, s02, s03, (half *) dst->data,
                       lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3,
                       dst->ne[0], dst->ne[1], dst->ne[2], dst->ne[3],
                       (bool) circular, stream);
    }
